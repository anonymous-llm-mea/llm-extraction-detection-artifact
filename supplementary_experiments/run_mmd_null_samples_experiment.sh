...
SEEDS="${SEEDS:-42}"
NULL_SAMPLES_LIST="${NULL_SAMPLES_LIST:-100 250 500 1000}"
...
EXPERIMENT_ROOT="${EXPERIMENT_ROOT:-supplementary_experiments/mmd_null_samples_experiment}"
...
SUMMARY_CSV="${SUMMARY_CSV:-${RESULT_ROOT}/mmd_null_samples_runs_bs${BATCH_SIZE}.csv}"
OVERALL_CSV="${OVERALL_CSV:-${RESULT_ROOT}/mmd_null_samples_overall_bs${BATCH_SIZE}.csv}"
printf "null_samples,dataset,seed,batch_size,reference_repeats,total_seconds,seconds_per_eval_unit,benign_fpr,5pct_tpr,10pct_tpr,25pct_tpr,50pct_tpr,100pct_tpr,avg_tpr,balanced_acc,output_dir,metadata_path,log_file,status\n" > "${SUMMARY_CSV}"
...
writer = csv.DictWriter(sys.stdout, fieldnames=[
    "null_samples", "dataset", "seed", "batch_size", "reference_repeats",
...
for row in rows:
    groups[row["null_samples"]].append(row)
fields = ["null_samples", "runs"]
...
with overall.open("w", newline="", encoding="utf-8") as f:
    ...
    for key, group in sorted(groups.items(), key=lambda item: float(item[0])):
        out = {"null_samples": key, "runs": str(len(group))}
        for metric in metrics:
            vals = []
            for row in group:
                try:
...
echo "Running MMD null-samples experiment..."
echo "Datasets: ${DATASETS}"
echo "Seeds: ${SEEDS}"
echo "Null samples: ${NULL_SAMPLES_LIST}"
echo "Summary CSV: ${SUMMARY_CSV}"
echo

for null_samples in ${NULL_SAMPLES_LIST}; do
  for seed in ${SEEDS}; do
    for dataset in ${dataset_items}; do
      [[ -z "${dataset}" ]] && continue
      warmup_embedding_cache "${dataset}" "${seed}"
      run_name="null${null_samples}/${dataset}_seed${seed}_bs${BATCH_SIZE}"
      output_dir="${OUTPUT_ROOT}/${run_name}"
      cache_dir="${SHARED_CACHE_DIR}"
      log_dir="${LOG_ROOT}/null${null_samples}"
      ...
      if [[ "${FORCE}" != "1" && -f "${metadata_path}" && -f "${time_file}" ]]; then
        ...
        append_summary "${null_samples}" "${dataset}" "${seed}" "${output_dir}" "${log_file}" "${time_file}" "0"
        continue
      fi
      echo "============================================================"
      echo "Null samples: ${null_samples}"
      ...
      status="$?"
      set -e
      append_summary "${null_samples}" "${dataset}" "${seed}" "${output_dir}" "${log_file}" "${time_file}" "${status}"
      if [[ "${status}" != "0" ]]; then
...
write_overall
echo
echo "Finished MMD null-samples experiment."
echo "Summary CSV: ${SUMMARY_CSV}"
